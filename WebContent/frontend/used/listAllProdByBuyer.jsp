...
<%	UsedService usedSvc = new UsedService();
	...
	buyerNo = new Integer(memberVO.getMemNo());
...
table.prods {
	border-top:3px #FFD382 solid;border-bottom:3px #82FFFF solid;
...
<table class="prods" border="1">
	...
	<c:forEach var="usedVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		<tr>
			<td>產品編號</td><td>${usedVO.usedNo}</td>
			</tr>
		<tr>
			<td>產品種類</td>
			<td>
			<c:if test="${usedVO.kindNo==1}">手機</c:if>
			<c:if test="${usedVO.kindNo==2}">電腦</c:if>
			<c:if test="${usedVO.kindNo==3}">手錶</c:if>
			<c:if test="${usedVO.kindNo==4}">相機</c:if>
			<c:if test="${usedVO.kindNo==5}">配件類</c:if>
			</td>
		</tr>
		<c:if test='${usedVO.buyerNo!=0}'>
		<tr>
			<td>買家編號</td><td>${usedVO.buyerNo}</td>
		</tr>
		</c:if>
		<tr>
			<td>產品名稱</td><td>${usedVO.usedName}</td>
		</tr>
		<tr>
			<td>產品價格</td><td>${usedVO.usedPrice}</td>
		</tr>
		<tr>
			<td>產品狀態</td>
			<td>
			<c:if test="${usedVO.usedState==1}">已下單</c:if>
			<c:if test='${usedVO.usedState==""}'>上架中</c:if>
			<c:if test='${usedVO.usedState==2}'>已出貨</c:if>
			<c:if test='${usedVO.usedState==3}'>已取消</c:if>
			<c:if test='${usedVO.usedState==4}'>已下架</c:if>
			</td>
		</tr>
		<tr>
			<td>上架時間</td><td><fmt:formatDate value="${usedVO.usedLaunchedTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
		</tr>
		<c:if test='${usedVO.soldTime!=null}'>
		<tr>
			<td>下架時間</td><td><fmt:formatDate value="${usedVO.soldTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
		</tr>
		</c:if>
		<c:if test='${usedVO.receiverName!=null}'>
		<tr>
			<td>收件人</td><td>${usedVO.receiverName}</td>
		</tr>
		</c:if>
		<c:if test='${usedVO.receiverAddress!=null}'>
		<tr>
			<td>收件地址</td><td>${usedVO.receiverAddress}</td>
		</tr>
		</c:if>
		<c:if test='${usedVO.receiverPhone!=null}'>
		<tr>
			<td>收件人電話</td><td>${usedVO.receiverPhone}</td>
		</tr>
		</c:if>
			<td>商品描述</td><td>${usedVO.usedProdDescription}</td>
		
		<tr class="buttom">

			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/used/usedServlet.do" style="margin-bottom: 0px;">
			  <label class="btn btn-info">預覽
			...
			<td>
			  ...
			  <label class="btn btn-outline-danger">取消
			     <input type="submit" style="display:none">
			   </label> 
			   </c:if>
			     <input type="hidden" name="usedNo"     value="${usedVO.usedNo}">
