...
<%
	...
	List<ItemVO> list = itemSvc.getOneKind3();
									...
									<c:if test="${itemVO.itemNo==itemPicVO.itemVO.itemNo}">
										<div class="card" style="width: 18rem;">
											...
											<jsp:useBean id="itemCollectionSvc" scope="page" class="com.itemCollection.model.ItemCollectionService" />
												<form  name="shoppingForm" action="<%=request.getContextPath()%>/getOneItemForViewFromListByCamera" method="POST">
										<input type="hidden" name="itemNo" value="${itemVO.itemNo}">
										<input type="hidden" name="action" value="getOneItemForViewFromListByCamera">
...
<!-- 										//TODO: 修改會員部分 -->
										...
										<c:choose>
										 <c:when test="${checkcount <= 0}">
											<a id ="collection-a" href="<%=request.getContextPath()%>/addCollectionFromCamera?memNo=${user.memNo}&itemNo=${itemVO.itemNo}">
