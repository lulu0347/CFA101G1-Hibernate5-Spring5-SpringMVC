...
<%
	...
	List<ItemVO> list = itemSvc.getOneKind2();
						...
						<c:forEach var="itemVO" items="${list}" begin="<%=pageIndex%>"
							...
							<td><c:forEach var="itemPicVO"
									...
									<c:if test="${itemVO.itemNo==itemPicVO.itemVO.itemNo}">
										...
										<form  name="shoppingForm" action="<%=request.getContextPath()%>/getOneItemForViewFromListByComputer" method="POST">
										<input type="hidden" name="itemNo" value="${itemVO.itemNo}">
										<input type="hidden" name="action" value="getOneItemForViewFromListByComputer">
...
<!-- 										//TODO: 修改會員部分 -->
										...
										<c:choose>
										 <c:when test="${checkcount <= 0}">
											<a id ="collection-a" href="<%=request.getContextPath()%>/addCollectionFromComputer?memNo=${user.memNo}&itemNo=${itemVO.itemNo}">
