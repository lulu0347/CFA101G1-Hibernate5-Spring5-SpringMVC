...
<%
	...
	List<ItemVO> list = itemSvc.getOneKind1();
					...
					<c:forEach var="itemPicVO" items="<%=itemPicSvc.getAllPics()%>">
						<c:if test="${itemVO.itemNo==itemPicVO.itemVO.itemNo}">
								<div class="card" style="width: 18rem;">
									...
									<form  name="shoppingForm" action="<%=request.getContextPath()%>/getOneItemForViewFromListByPhone" method="POST">
									<input type="hidden" name="itemNo" value="${itemVO.itemNo}">
									<input type="hidden" name="action" value="getOneItemForViewFromListByPhone">
...
<!-- 										//TODO: 修改會員部分 -->
										...
										<c:choose>
										 <c:when test="${checkcount <= 0}">
											<a id ="collection-a" href="<%=request.getContextPath()%>/addCollectionFromPhone?memNo=${user.memNo}&itemNo=${itemVO.itemNo}">
